...
public class ConnectTask extends AsyncTask<Void, Void, Void> {
	...
	public ConnectTask(Context context, Data data, TextView body_tv, TextView error_tv) {
	...
	protected void onProgressUpdate(Void... values) {
		...
		// 数据更新
		if (data.getBody() != null && data.getBody() == true) {
			body_tv.setText("有人");
		} else {
			body_tv.setText("无人");
		}
	...
	protected Void doInBackground(Void... params) {
		...
		try {
			...
			while (CIRCLE) {
				// 查询
				StreamUtil.writeCommand(outputStream, Constant.BODY_CHK);
				Thread.sleep(200);
				read_buff = StreamUtil.readData(inputStream);
				body = FROBody.getData(Constant.BODY_LEN, Constant.BODY_NUM, read_buff);
				if (body != null) {
					data.setBody(body);
				}
