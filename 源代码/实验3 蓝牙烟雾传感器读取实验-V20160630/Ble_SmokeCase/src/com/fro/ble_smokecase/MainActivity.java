...
import com.fro.ble_smokecase.BluetoothLeClass.OnDataAvailableListener;
import com.fro.ble_smokecase.BluetoothLeClass.OnServiceDiscoverListener;
...
//			Log.i(TAG, "初始化BLE失败");
	...
	private BluetoothLeClass.OnDataAvailableListener mOnDataAvailable = new OnDataAvailableListener() {
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
			...
			if (characteristic.getUuid().toString().equals(UUID_CHAR6)) {
				...
				// 接收数据
				Float fData=FROSmoke.getData(Const.SMOKE_LEN, Const.NODE_NUM, characteristic.getValue());
				String value =String.valueOf((int)(float)fData);
	...
	private void ReadBle() {
		...
		if (mBLE.getmBluetoothGatt() != null && gattCharacteristic_char6!= null) {
			// 获取发送命令
			String writeStr = Const.SMOKE_CMD;
			// 设置数据内容
